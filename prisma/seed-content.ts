...
async function seedContent() {
  ...
  const sscBiology = sscClass ? await db.subject.findFirst({ where: { slug: 'biology', classId: sscClass.id } }) : null
  const sscBangla = sscClass ? await db.subject.findFirst({ where: { slug: 'bangla', classId: sscClass.id } }) : null
  const sscEnglish = sscClass ? await db.subject.findFirst({ where: { slug: 'english', classId: sscClass.id } }) : null

  const hscPhysics = hscClass ? await db.subject.findFirst({ where: { slug: 'physics', classId: hscClass.id } }) : null
  const hscChemistry = hscClass ? await db.subject.findFirst({ where: { slug: 'chemistry', classId: hscClass.id } }) : null
  const hscMath = hscClass ? await db.subject.findFirst({ where: { slug: 'higher-math', classId: hscClass.id } }) : null
  const hscBangla = hscClass ? await db.subject.findFirst({ where: { slug: 'bangla', classId: hscClass.id } }) : null
  ...
  if (sscClass && sscPhysics) {
    const sscCqPkgSlug = 'ssc-physics-cq-practice'
