...
export function useMCQExamPackages() {
  ...
  const [leaderboardSetId, setLeaderboardSetId] = useState<string | null>(null)
  ...
  const fetchPackages = useCallback(async () => {
    setLoading(true)
    try {
      const params: any = { action: 'list', page: '1', limit: '50' }
  ...
  const handleSearchMcqs = useCallback(async () => {
    ...
    try {
      const params: any = { action: 'search-mcqs', page: '1', limit: '30' }
