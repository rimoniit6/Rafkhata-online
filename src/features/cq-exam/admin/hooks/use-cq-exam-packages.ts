...
async function unwrapResponse<T = any>(url: string, options?: RequestInit): Promise<T> {
...
export function useCQExamPackages() {
  ...
  const fetchPackages = useCallback(async () => {
    ...
    try {
      ...
      const data: any = await unwrapResponse(`/api/admin/cq-exam-packages?${params}`)
  ...
  const fetchPackageDetail = useCallback(async (packageId: string) => {
    setLoading(true)
    try {
      const data: any = await unwrapResponse(`/api/admin/cq-exam-packages?action=detail&id=${packageId}`)
  ...
  const fetchSetDetail = useCallback(async (setId: string) => {
    setLoading(true)
    try {
      const data: any = await unwrapResponse(`/api/admin/cq-exam-packages?action=set-detail&setId=${setId}`)
  ...
  const fetchSubmissions = useCallback(async (setId: string) => {
    setLoading(true)
    try {
      const data: any = await unwrapResponse(`/api/admin/cq-exam-packages?action=submissions&setId=${setId}`)
  ...
  const fetchSubmissionDetail = useCallback(async (submissionId: string) => {
    setLoading(true)
    try {
      const data: any = await unwrapResponse(`/api/admin/cq-exam-packages?action=submission-detail&submissionId=${submissionId}`)
  ...
  const handleSavePackage = async () => {
    ...
    try {
      ...
      toast({ title: 'ত্রুটি', description: err?.message || 'নেটওয়ার্ক সমস্যা', variant: 'destructive' })
    } finally {
      setSaving(false)
    }
  }
  ...
  const handleFetchBulkSubmissions = useCallback(async (questionId: string) => {
    ...
    try {
      const data: any = await unwrapResponse(
  ...
  const handleBulkGrade = async (setId: string, defaultMarks: number = 0) => {
    setSaving(true)
    try {
      const data: any = await unwrapResponse('/api/admin/cq-exam-packages', {
        method: 'PUT',
  ...
  const openLeaderboard = async (setId: string, setTitle: string) => {
    ...
    try {
      const data: any = await unwrapResponse(`/api/admin/cq-exam-packages?action=submissions&setId=${setId}`)
      const sorted = (data.submissions || []).sort((a: any, b: any) => (b.obtainedMarks || 0) - (a.obtainedMarks || 0))
