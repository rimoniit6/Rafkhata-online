...
import { apiResponse, paginatedApiResponse, apiError, withAdmin, withCsrf } from '@/lib/api-utils'
...
if ('error' in csrfCheck) return csrfCheck.error
    ...
    if (!type || (type !== 'mcq' && type !== 'cq')) {
      return apiError('প্রশ্নের ধরন নির্ধারণ করুন (mcq বা cq)', 400)
    }

    if (type === 'mcq') {
      const { question, questionImage, optionA, optionAImage, optionB, optionBImage, optionC, optionCImage, optionD, optionDImage, correctAnswer, explanation, explanationImage, chapterId, classLevel, subjectId, board, year, topic, difficulty, isPremium, price, tags, isActive } = fields

      if (!question || !optionA || !optionB || !optionC || !optionD || !correctAnswer || !chapterId || !classLevel || !subjectId) {
        return apiError('প্রয়োজনীয় ফিল্ড পূরণ করুন (প্রশ্ন, অপশন, সঠিক উত্তর, অধ্যায়, শ্রেণি, বিষয়)', 400)
      }
    ...
    const { uddeepok, uddeepokImage, question1, question1Image, question2, question2Image, question3, question3Image, question4, question4Image, answer1, answer1Image, answer2, answer2Image, answer3, answer3Image, answer4, answer4Image, chapterId, classLevel, subjectId, board, year, topic, difficulty, isPremium, price, tags, isActive } = fields
...
if ('error' in csrfCheck) return csrfCheck.error
    ...
    if (type === 'mcq') {
      ...
      const updated = await db.mCQ.update({ where: { id }, data })
      await invalidateContentCache('board-question')
    ...
    const updated = await db.cQ.update({ where: { id }, data })
    await invalidateContentCache('board-question')
    return apiResponse(updated)
  } catch (error) {
