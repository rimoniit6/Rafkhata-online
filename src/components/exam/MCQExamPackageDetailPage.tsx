...
export default function MCQExamPackageDetailPage() {
  ...
  const [examSetStatusLoading, setExamSetStatusLoading] = useState(false)
  ...
  const [retakeHistoryLoading, setRetakeHistoryLoading] = useState(false)
  ...
  useEffect(() => {
    ...
    const calculateCountdown = () => {
      const now = new Date()
      const [sh, sm] = (todaySet.startTime || '00:00').split(':').map(Number)
      const startDate = new Date(now)
      startDate.setHours(sh, sm, 0, 0)
      const diff = Math.max(0, Math.floor((startDate.getTime() - now.getTime()) / 1000))
