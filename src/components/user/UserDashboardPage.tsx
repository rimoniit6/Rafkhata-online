...
export default function UserDashboardPage() {
  ...
  const [recentLectures, setRecentLectures] = useState<Array<{
    id: string; title: string; subject: string; chapter: string; progress: number; viewedAt: string
  }>>([])

  useEffect(() => {
    fetch('/api/user/recent-lectures')
      .then(r => r.ok ? r.json() : { data: [] })
      .then(j => setRecentLectures(Array.isArray(j.data) ? j.data : []))
      .catch(() => {})
  }, [])
  ...
  const bookmarkedQuestions = realBookmarks.map(b => ({
    ...
    text: b.contentTitle,
    type: b.contentType as 'mcq' | 'cq' | 'lecture',
  }))
          ...
          <div className="flex items-center gap-5">
            ...
            <div className="flex-1 min-w-0">
              ...
              <motion.p
                ...
                className="text-emerald-100/80 text-sm mt-0.5 flex items-center gap-2"
              >
